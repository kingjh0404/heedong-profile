...
export default function ProjectsPage() {
  ...
  const projects = [
    ...
    },
    {
      ...
      link: "https://www.youtube.com/watch?v=RIzuujA8qvc&t=34m40s",
      linkDescription: "34분 40초 ~ 36분 23초",
      github: "https://github.com",
      highlights: [
        "3D 공간 내 길이 및 거리 측정",
        "작업자 위치 추정 및 시각화",
      ...
      details: {
        ...
        role: "프론트 엔드, 3D 시각화",
        contributions: [
          "실험동 3D 디지털 트윈 뷰어 및 거리 측정·위치 시각화 기능 전체 설계",
          "위험 요소·안전 동선 편집을 위한 웹 기반 저작 도구 UI/UX 설계 및 구현",
          "AI 비전 기반 안전 크래딧 결과를 실시간으로 반영하는 대시보드 개발",
        ],
        description:
          "본 프로젝트는 실험동 공장 작업 환경을 디지털 트윈으로 재현하고, 작업자가 사전에 안전 교육을 받고 작업 계획을 수립할 수 있도록 지원하는 시스템입니다. 드론으로 촬영한 현장 데이터를 3D Gaussian Splatting 기법으로 처리하여 고품질의 3D 모델을 구축하였으며, 주요 기능으로는 실험실 환경의 3D 디지털 트윈 구현, 3D 공간 내 길이 및 거리 측정 기능, 작업자 실시간 위치 정밀 추정 및 시각화 기능이 포함됩니다.\n\n위험 요소 시각화 및 안전 동선 시뮬레이션을 위해 관리자가 안전 표시, 위험 표시, 경고 메시지 등을 생성, 배치, 수정할 수 있는 저작 도구를 제공하며, 이 저작 도구에는 안전 교육 동영상을 업로드하여 배포할 수 있는 기능이 통합되어 있습니다. 특히, 3D 공간 내 특정 지점에 스티커 형태로 작업 현황이나 위험 사항을 직접 작성하고 부착하여 현장 정보를 직관적으로 공유하는 기능도 구현되었습니다.\n\n작업자가 가상 환경에서 실제 위험 요소를 체험하고 표준 안전 절차를 숙달할 수 있도록 몰입형 작업 시뮬레이션 콘텐츠 개발을 지원하며, 안전 강화를 위한 작업 안전 강화 및 피드백 시스템(안전 크래딧 기능)을 도입하여 TBM 작성 여부를 확인하고, AI 비전 기반으로 작업자의 안전모, 장화, 헬멧 등의 개인 보호 장비 착용 여부를 자동으로 검사하여 크래딧을 부여함으로써 교육생의 안전 의식을 실질적으로 향상시킵니다.",
        features: [
          "3D 공간 내 길이 및 거리 측정",
          "작업자 위치 추정 및 시각화",
          "안전 콘텐츠 통합 저작 도구",
    ...
    },
    {
      ...
      details: {
        ...
        team: "2명",
        role: "PM, 풀스택 개발, 3D 시각화",
        contributions: [
          "AI 모델 → TCP 스트림 → WebSocket → WebGL까지 이어지는 데이터 파이프라인 설계",
          "3D GS 기반 설비 모니터링 화면 및 실시간 알람/센서 데이터 시각화 구현",
          "RTSP CCTV, 센서, 알람 등 이종 실시간 데이터를 하나의 3D UI로 통합하는 구조 설계",
        ],
        description:
          "이 프로젝트는 AI 모델이 생성하는 3D 공간 데이터와 다양한 실시간 정보를 웹에서 시각화하는 3D 모니터링 솔루션입니다. 드론으로 촬영한 이미지·영상을 기반으로 구축한 3D 가우시안 스플래팅 모델을 사용하며, AI 모델이 출력하는 데이터는 TCP 기반 실시간 스트림 형태로 시스템에 전달됩니다. 전달된 데이터는 즉시 처리되어 WebSocket을 통해 웹 애플리케이션으로 전송되고, WebGL 렌더링 엔진이 이를 반영해 사용자는 브라우저만으로도 고품질 3D 공간을 실시간 탐색할 수 있습니다.\n\n여기에 3D 데이터 외에도 다양한 실시간 정보가 통합되어 있습니다. RTSP 기반 CCTV 실시간 영상 스트리밍, 환경 및 설비 관련 실시간 센서 데이터 표시, 위험 상황 발생 시 경보 및 알림 제공 등이 포함됩니다. 이 모든 요소를 하나의 3D UI에서 확인할 수 있어, 공간 정보가 시간 정보와 결합된 실시간 디지털 트윈 모니터링 환경을 제공합니다.\n\n프로젝트 전반을 통해 AI 모델에서 생성되는 데이터 흐름을 웹으로 자연스럽게 연결하고, WebSocket 기반 실시간 전달, WebGL 기반 고성능 3D 표현까지 구현해 현실 공간을 온라인에서 실시간으로 재현하고 이해할 수 있는 기술적 기반을 마련했습니다.",
    ...
    },
    {
      ...
      details: {
        ...
        role: "프론트엔드",
        contributions: [
          "Spot·와이어볼 로봇 관제 화면 전체 HMI 레이아웃 및 인터랙션 설계",
          "로봇 상태(배터리, 통신, 임무 상태) 실시간 표시 컴포넌트 구현",
          "원격 제어·자율 미션 설정 플로우를 웹 UI로 설계하고 제어 명령 연동",
        ],
        description:
          "본 프로젝트는 포스코 포항제철소의 현장 안전성 및 설비 점검 운영 효율 극대화를 핵심 목표로 하여, 로봇 플랫폼을 통합 관제하고 제어할 수 있는 고성능 웹 기반 HMI 시스템을 개발한 것입니다. 시스템의 주요 성과는 보스턴 다이나믹스의 4족 보행 로봇 Spot의 기능을 웹 환경으로 완벽하게 통합한 데 있습니다. 개발된 웹페이지를 통해 작업자는 현장에 직접 가지 않고도 Spot 로봇을 정밀하게 원격 제어할 수 있으며, 직관적인 인터페이스 내에서 순찰 경로와 점검 항목을 설정하여 자율 미션을 생성하고 부여합니다.\n\nSpot 로봇은 미션 수행 중 탑재된 첨단 센서를 가동하여 핵심 설비의 상태를 진단합니다. 특히, 설비의 미세한 온도 변화를 감지하기 위해 설비 온도를 정확하게 측정하며, 이와 함께 열화상 카메라 및 고화질 일반 카메라를 통해 수집된 고품질 이미지와 데이터를 웹 인터페이스로 실시간 전송 및 시각화합니다. 관제 화면에서는 로봇의 현재 배터리 상태, 통신 상태, 그리고 임무 상태(충전 중, 미션 수행 중, 대기 중 등)를 명확하게 표시하여 운영자가 로봇 상태를 한눈에 파악하고 신속하게 대응할 수 있도록 관제 편의성을 극대화했습니다.\n\n또한, 이 HMI 시스템은 와이어볼(Wireball) 로봇까지 관제 대상을 확장하여 다종 로봇 운영의 효율을 높였습니다. 작업자는 웹 화면에서 와이어볼 로봇의 실시간 위치 정보를 정확하게 파악할 수 있으며, 상세한 운영 상태와 배터리 잔량을 포함한 핵심 데이터를 모니터링합니다. RTSP와 WebRTC 기술을 활용한 초저지연 영상 스트리밍으로 원격 제어의 안정성을 확보하여, 제철소라는 복잡하고 위험한 환경에서 로봇의 안정적이고 효율적인 운용을 가능하게 했습니다.",
    ...
    },
    {
      ...
      details: {
        ...
        contributions: [
          ...
          "실시간 영상 스트림을 웹에서 안정적으로 재생하기 위한 뷰어 구성",
        ],
        description:
          "본 프로젝트는 포스코 포항제철소 현장 점검 및 순찰 임무를 위해 투입된 HUSKY 무인 이동 로봇을 대상으로, 효율적인 운영과 관리를 위한 웹 기반 관제 프로그램을 개발한 것입니다. 시스템의 핵심 기능은 웹을 통한 로봇의 원격 제어 및 자율 임무 설정에 있습니다. 작업자는 현장에 직접 가지 않고도 웹 인터페이스를 통해 HUSKY 로봇을 직접 조종할 수 있으며, 복잡한 순찰 경로 및 작업 지점을 지정하여 미션을 생성하고 로봇에게 부여할 수 있습니다. 로봇은 이 미션을 받아 자율적으로 이동 및 작업을 수행합니다.\n\n관제 시스템의 중요한 성과는 로봇의 실시간 위치 정보 시각화에 있습니다. HUSKY 로봇의 현재 위치는 제철소 현장 지도를 기반으로 하는 2D 환경 내에 정확하게 표시됩니다. 이를 통해 운영자는 로봇이 현장의 어느 지점에서 임무를 수행하고 있는지 한눈에 파악할 수 있어 관제 효율성을 높입니다.\n\n또한, 로봇의 현장 시야를 확보하기 위해 HUSKY 로봇에 탑재된 카메라의 실시간 영상 스트림을 처리했습니다. 이 영상은 카메라의 URL을 직접 받아 웹페이지에 표현함으로써, 운영자는 로봇의 시점에서 현장 상황을 실시간으로 확인하고 필요 시 즉각적인 제어 및 미션 수정이 가능하도록 시스템을 구축했습니다. 궁극적으로 이 HUSKY 로봇 관제 프로그램은 제철소 현장 점검의 안전성과 신속성을 보장하며, 작업자의 현장 투입을 최소화하여 효율적인 무인 시스템 운영 환경을 제공합니다.",
    ...
    },
    {
      ...
      tags: [ "React","Node.js", "JavaScript", "Three.js", "3DGS"],
      link: "https://example.com",
      github: "https://github.com",
      ...
      details: {
        ...
        role: "프론트 엔드, 3D 시각화",
        contributions: [
          "열차·용선 물류 디지털 트윈 3D 시각화 화면 설계 및 구현",
          "실시간 열차 위치 및 물류 지표를 통합한 모니터링 대시보드 개발",
          "Kafka·MongoDB·MariaDB에서 오는 데이터를 프론트에서 효율적으로 소비할 수 있는 API 설계 협업",
        ],
        description:
          "포스코 포항제철소의 용선(쇳물) 물류 운영을 디지털 트윈으로 재현하여 물류 비용을 절감하고 운영 효율성을 극대화하기 위해 진행된 프로젝트입니다. 기존에는 열차 배차와 물류 운영이 수동적이고 경험에 의존하는 방식으로 진행되어 비효율과 비용 손실이 발생했지만, 실시간 데이터 기반의 의사결정을 통해 이러한 문제를 해결하고자 했습니다.\n\n드론으로 촬영한 현장 데이터를 3D Gaussian Splatting 기법으로 처리하여 실제 공정 환경을 정확하게 재현한 디지털 트윈을 구축했습니다. 포스코 사내 EAI 통신과 열차 GPS 데이터를 실시간으로 연동하여 실제 열차의 움직임을 가상 공간에서 실시간으로 추적하고 시각화할 수 있도록 했습니다. 이를 통해 운영자는 복잡한 현장 상황을 한눈에 파악하고 신속한 의사결정을 내릴 수 있게 되었습니다.\n\n단순한 시각화를 넘어 AI 기반 예측 모델을 통합하여 용선 생산량과 고로 출선 폐쇄 시점을 예측하고, 이를 바탕으로 열차 배차를 최적화하는 시스템을 구축했습니다. 실시간 데이터 스트리밍과 대용량 데이터 처리를 위한 아키텍처를 설계하여 운영 의사결정의 정확성과 속도를 크게 향상시켰으며, 결과적으로 물류 운영 비용 절감과 효율성 개선에 기여했습니다.",
    ...
    },
    {
      ...
      details: {
        ...
        contributions: [
          ...
          "Fail-safe 수동 제어 플로우를 고려한 제어 패널 인터페이스 구현",
        ],
        description:
          "본 프로젝트는 기차 단열커버 자동 개폐 시스템의 디지털 운영 환경을 구현하여, 현장 운영자의 업무 효율성과 안전성을 향상시키는 것을 목표로 진행되었습니다. Kafka 기반 실시간 데이터 스트리밍을 통해 기차 위치, 배터리 상태, 통신 품질, 명령 수행 결과 등의 데이터를 수집하고, Node.js 백엔드에서 이를 처리하여 운영에 필요한 정제된 정보를 제공합니다.\n\n프론트엔드는 이러한 데이터를 WebSocket 및 REST API를 통해 수신하여 실시간으로 시각화하는 대시보드를 구축해, 기차 상태를 직관적으로 확인하고 장애를 사전에 예방할 수 있는 사용자 경험을 제공했습니다. 또한 명령 이력 및 이벤트 로그 관리 기능을 포함하여 운영자가 특정 시점의 자동화 결정 과정을 추적하고 검증할 수 있도록 설계되었습니다.\n\n비상 상황 발생 시 자동 제어를 일시 중지하고 운영자가 수동으로 개폐 명령을 내릴 수 있는 Fail-safe 기능을 포함해, 자동화 시스템의 안정성과 신뢰성을 강화했습니다. 더불어 사용자 권한 기반의 설정 기능을 제공하여 보안과 운영 통제를 확보했습니다. 프론트엔드와 백엔드의 안정적인 통합을 통해 운영자는 복잡한 자동 개폐 시스템을 실시간으로 모니터링하고 정확하게 제어할 수 있게 되었습니다.",
            ...
            {projects.map((project, index) => (
              <Dialog key={index}>
                <DialogTrigger asChild>
                  <Card className="overflow-hidden flex flex-col gap-2 transition-all duration-300 hover:scale-105 hover:shadow-[0_0_30px_rgba(135,206,235,0.6)] hover:border-primary cursor-pointer py-0">
                    {/* Project Image */}
                    ...
                    {/* Project Details */}
                    <div className="px-3 pb-3 pt-0 sm:pt-3 flex flex-col flex-1">
                  ...
                  <div className="grid md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
                    ...
                    <div className="space-y-6 animate-fade-in-up" style={{ animationDelay: '0.1s', animationFillMode: 'both' }}>
                      ...
                      <div className="space-y-2 sm:space-y-3 text-xs sm:text-sm">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 md:gap-6 flex-wrap">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="font-semibold text-primary">역할:</span>
                            <span className="break-keep">{project.details.role}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      {project.details.contributions && project.details.contributions.length > 0 && (
                        <div>
                          <h3 className="font-semibold text-base sm:text-lg mb-2 sm:mb-3 text-primary">담당 파트 및 구현</h3>
                            ...
                            {project.details.achievements.map((achievement, i) => {
                              ...
                              return (
                                <li key={i} className="break-keep">
                                  {parts.map((part, idx) => 
                                    /\d+%/.test(part) ? (
                                      <span key={idx} className="font-bold text-green-600 dark:text-green-400">{part}</span>
                                    ) : (
                                      part
                                    )
                                  )}
                      ...
                      {project.link && project.link !== "https://example.com" && (
                        <div className="mt-4">
                          <a
                            ...
                            className="inline-flex items-center gap-2 px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg transition-colors text-sm sm:text-base font-medium"
                          >
                            ...
                            {project.linkDescription && (
                              <span className="text-xs">({project.linkDescription})</span>
                            )}
                          </a>
                        </div>
                      )}
